...
import static com.faforever.client.config.CacheNames.MATCHMAKER_QUEUE_MAP_POOLS;
...
import static com.faforever.client.config.CacheNames.PLAYERS_BY_ID;
...
public class CacheConfig extends CachingConfigurerSupport {
  ...
  public CacheManager cacheManager() {
    ...
    simpleCacheManager.setCaches(Arrays.asList(
        ...
        new CaffeineCache(LEADERBOARD, newBuilder().expireAfterWrite(1, MINUTES).build()),
        new CaffeineCache(GLOBAL_LEADERBOARD, newBuilder().maximumSize(1).expireAfterAccess(1, MINUTES).build()),
        new CaffeineCache(LADDER_1V1_LEADERBOARD, newBuilder().maximumSize(1).expireAfterAccess(1, MINUTES).build()),
        ...
        new CaffeineCache(MATCHMAKER_QUEUE_MAP_POOLS, newBuilder().expireAfterWrite(24, HOURS).build()),
        new CaffeineCache(MATCHMAKER_POOLS, newBuilder().expireAfterWrite(1, HOURS).build()),
        new CaffeineCache(ALL_RANKED_MAPS, newBuilder().expireAfterWrite(24, HOURS).build()),
